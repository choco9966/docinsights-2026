...
Validation 217개 PDF를 이미지로만 읽어 검증한 Codex 전사를 `codex-assisted-silver` reference로 사용해 Apple Vision과 Tesseract OCR 텍스트를 전수 채점했다. 이 평가는 Golden Label과 같은 계산 방향을 제공하지만 reference 자체가 사람이 만든 gold가 아니므로 결과 이름과 JSON 계약에 `silver_agreement_not_human_gold_accuracy`를 강제한다.
...
| 엔진 | 모델/weight 크기 | 성공 | Silver score | micro CER | micro WER | 문자 유사도 | NFKC+공백 유사도 | block exact | exact-token F1 | quantity F1 | 평균 sec/doc | docs/min | p95 sec/doc | process max RSS |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Tesseract 5.5.3 `eng`, PSM 6, 200 DPI | `eng.traineddata` 3.9MiB; Homebrew 설치 36MiB | 217/217 | **99.9415** | 0.000585 | 0.006029 | 0.999415 | 0.999415 | 217/217 | 0.992057 | 0.991993 | 6.0335 | 9.9445 | 6.3423 | 107.531MiB |
| Apple Vision accurate, 200 DPI | OS 내장 모델이라 weight 분리 측정 불가; runner 162KiB | 217/217 | **99.3777** | 0.006223 | 0.008946 | 0.993800 | 0.993799 | 217/217 | 0.985062 | 0.981604 | 3.8781 | 15.4714 | 4.0986 | 214.484MiB |

Codex silver 기준에서는 Tesseract가 문자·단어·critical quantity agreement가 높고 Apple Vision이 1.56배 빠르다. 이 결과는 사람 gold 정확도 순위가 아니다. 두 엔진 모두 동일 문서 패턴이나 renderer에 의해 같은 오류를 낼 수 있고 Codex도 전사 오류를 포함할 수 있다.
...
동일 Mac/200 DPI 조건에서 portable primary는 Tesseract를 유지한다. Apple Vision은 처리량 우위가 있어 macOS challenger/fallback으로 유지한다. 단, canonical 엔진 변경이나 대회 정확도 주장은 독립 human gold 또는 Qwen end-to-end ablation으로 다시 확인해야 한다.

## 독립 검수

별도 읽기 전용 verifier가 reference/prediction source 해시, 217 unique instance, score/CER/WER/F1/latency/block aggregate를 instance 행에서 독립 재계산했다. Apple Vision과 Tesseract의 JSON/Markdown을 fresh CLI 실행한 결과 기존 네 artifact와 byte-exact 일치했다. `edit_distance`는 무작위 문자열 20,000쌍을 독립 dynamic programming 구현과 대조해 불일치 0건이었다.

source overwrite 방지 회귀 테스트를 포함한 최종 검증은 `182 passed`, Ruff 통과, `git diff --check` 통과이며 verifier 판정은 `APPROVED`다. 설치된 JSON Schema validator는 없어 schema JSON parse, required/constant/range와 실제 artifact shape를 직접 대조했다.
